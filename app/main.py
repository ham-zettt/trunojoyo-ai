...
# Model configuration - FIXED: menggunakan 128 sesuai dengan model yang dilatih
LATENT_DIM = 128  # <-- CHANGED FROM 100 TO 128
IMAGE_SIZE = 64
CHANNELS = 3

class Generator(nn.Module):
    def __init__(self, z_dim=128, img_channels=3, g_channels=64):  # <-- FIXED: default 128
...
# Update di main.py untuk menerima noise_dim parameter
@app.post("/api/generate_batik")
async def generate_batik(request_data: dict):
    """Generate batik images using the trained model"""
    try:
        ...
        for i, img_tensor in enumerate(generated_images):
            ...
            file_path = save_image(img_tensor, filename)
            
            if base64_img and file_path:
                results.append({
        ...
        return {
            "status": "success", 
            "message": f"Generated {len(results)} batik images successfully",
            "images": results,
            "parameters": {
...
async def model_status():
    ...
    return {
        ...
        "latent_dim": LATENT_DIM  # <-- Always 128
    }
